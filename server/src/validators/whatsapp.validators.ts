...
export const aiConfigurationInput = z.object({
  ...
  // Lead scoring points and band thresholds (spec §5), tunable from AI Settings.
  leadScoring: z.object({
    requestedPricing: z.coerce.number().min(0).max(100).optional(),
    requestedDemo: z.coerce.number().min(0).max(100).optional(),
    requestedQuotation: z.coerce.number().min(0).max(100).optional(),
    providedBudget: z.coerce.number().min(0).max(100).optional(),
    timelineUnder30Days: z.coerce.number().min(0).max(100).optional(),
    providedCompany: z.coerce.number().min(0).max(100).optional(),
    providedEmail: z.coerce.number().min(0).max(100).optional(),
    repeatedEngagement: z.coerce.number().min(0).max(100).optional(),
    requestedHuman: z.coerce.number().min(0).max(100).optional(),
    warmThreshold: z.coerce.number().min(0).max(100).optional(),
    qualifiedThreshold: z.coerce.number().min(0).max(100).optional(),
    hotThreshold: z.coerce.number().min(0).max(100).optional(),
  }).optional(),

...
type AdminUser = Named & { email:string; active:boolean; role:Named; createdAt:string; isSystem?:boolean };
...
function UserAccess({onInvite,isAdmin}:{onInvite:()=>void;isAdmin:boolean}){
  ...
  return <div className="max-w-5xl space-y-4">
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><RoleInfo title="Administrator" detail="Full CRM access, configuration, reports, and user management."/><RoleInfo title="Sales Manager" detail="Access to their own records and teams they lead or belong to."/><RoleInfo title="Salesperson" detail="Access primarily to leads and opportunities assigned to them."/><RoleInfo title="AI Agent" detail="Protected system identity used for WhatsApp automation and AI-created activity history."/></div>
    ...
    <div className="panel overflow-hidden"><div className="flex items-center border-b bg-slate-50 px-4 py-3"><div><h2 className="text-sm font-semibold">Users with access</h2><p className="text-xs text-slate-500">Active users can sign in. The WhatsApp AI Agent is a protected automation account and cannot sign in.</p></div>{isAdmin&&<Button className="ml-auto" onClick={onInvite}><UserPlus size={14}/>Invite person</Button>}</div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead className="border-b text-slate-500"><tr><th className="p-3">User</th><th>Email</th><th>Role</th><th>Status</th>{isAdmin&&<th className="pr-3 text-right">Actions</th>}</tr></thead><tbody>{users.data?.map(user=><tr className="border-b" key={user._id}><td className="p-3"><span className="flex items-center gap-2"><Avatar name={user.name}/><b>{user.name}</b>{user.isSystem&&<span className="badge bg-violet-50 text-violet-700">System</span>}</span></td><td>{user.email}</td><td>{user.isSystem?<span className="badge bg-slate-100 text-slate-700">{user.role?.name||'AI Agent'}</span>:<select className="field w-44" value={user.role?._id||''} disabled={update.isPending} onChange={event=>update.mutate({id:user._id,role:event.target.value})}>{roles.data?.filter(role=>role.name!=='AI Agent').map(role=><option key={role._id} value={role._id}>{role.name}</option>)}</select>}</td><td>{user.isSystem?<span className="badge bg-violet-50 text-violet-700">Automation only</span>:<button type="button" className={`badge ${user.active?'bg-emerald-50 text-emerald-700':'bg-amber-50 text-amber-700'}`} disabled={update.isPending||signedInUser?._id===user._id} title={signedInUser?._id===user._id?'You cannot suspend your own account':'Click to change access'} onClick={()=>update.mutate({id:user._id,active:!user.active})}>{user.active?'Active':'Suspended'}</button>}</td>{isAdmin&&<td className="pr-3 text-right">{!user.isSystem&&signedInUser?._id!==user._id&&<RowMenu label="Delete user" busy={removeUser.isPending} onDelete={()=>{if(confirm(`Delete ${user.name}? Their login will be removed, their profile anonymized, and assignments cleared. Historical audit records will remain.`))removeUser.mutate(user._id)}}/>}</td>}</tr>)}</tbody></table></div></div>
...
function RoleAdmin({isAdmin}:{isAdmin:boolean}){
  ...
  const builtIn=new Set(['Administrator','Sales Manager','Salesperson','AI Agent']);
